...
const ReviewResultDisplay: React.FC<Props> = ({ result, onHybridAnalyze, lang }) => {
  ...
  // Normalize S_adj to 100-scale if model returns 0-5 scale
  const rawSAdj = result.indices?.S_adj ?? 0;
  // If score is <= 10, it's likely on 0-5 scale, so multiply by 20.
  // This handles the user report where model outputs 4.30 instead of 86.00
  const displaySAdj = rawSAdj <= 10 ? rawSAdj * 20 : rawSAdj;
  ...
  const handleExport = async () => {
    ...
    try {
        const canvas = await html2canvas(resultRef.current, {
            ...
            useCORS: true,
            logging: false,
            // Remove windowWidth constraint to let it flow naturally, 
            // but we control the container width in onclone
            onclone: (clonedDoc) => {
                const element = clonedDoc.querySelector('[data-export-container="true"]');
                if (element) {
                    const el = element as HTMLElement;
                    
                    // --- ADAPTIVE LAYOUT STRATEGY ---
                    // 1. Set a "Golden Mean" width. 900px is wide enough for desktop grid
                    //    but narrow enough to keep text dense.
                    el.style.width = '900px'; 
                    el.style.maxWidth = 'none';
                    el.style.margin = '0';
                    el.style.padding = '40px'; 
                    
                    // 2. CRITICAL: Force Height to AUTO to prevent overlapping/stacking
                    //    If the app was in a scroll container, html2canvas might clip it.
                    el.style.height = 'auto';
                    el.style.minHeight = 'fit-content';
                    el.style.overflow = 'visible';

                    // 3. Fix Chart Height specifically
                    const chartContainers = el.querySelectorAll('.recharts-responsive-container');
                    chartContainers.forEach((c: any) => {
                        if (c.parentNode) {
                            c.parentNode.style.height = '350px';
                            c.parentNode.style.width = '100%';
                        }
                    });

                    // 4. Force Grid Gap to ensure separation
                    const grids = el.querySelectorAll('.grid');
                    grids.forEach((g: any) => {
                        g.style.gap = '24px'; // Force consistent gap
                    });
                }
            }
        });
        ...
        link.download = `X-Critical-${result.title_localized?.en?.replace(/\s+/g, '_') || 'Analysis'}.jpg`;
  ...
  return (
    <div className={`w-full max-w-5xl mx-auto p-1 md:p-6 animate-fade-in pb-24`}>
      ...
      <div ref={resultRef} data-export-container="true" className="bg-obsidian p-1 rounded-xl">
          ...
          <div className={`relative overflow-hidden rounded-xl border-2 ${borderColor} bg-obsidian mb-6 shadow-2xl transition-all duration-500`}>
            ...
            {/* Export Button (Hidden in Screenshot) */}
            <div className="absolute top-6 right-6 z-50" data-html2canvas-ignore="true">
                <button 
                    onClick={handleExport}
                    disabled={isExporting}
                    className="flex items-center gap-2 px-3 py-1.5 bg-zinc-900/80 hover:bg-gold hover:text-black border border-zinc-700 rounded text-xs text-zinc-300 transition-all font-mono"
                >
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-6">
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-2 flex-wrap">
                        ...
                        <span className="text-xs text-zinc-500 border border-zinc-700 px-2 py-0.5 rounded font-mono whitespace-nowrap">
                            {t.basis}: TYPE-{result.verdict_basis}
                        </span>
                        )}
                        <span className={`text-xs px-2 py-0.5 rounded font-mono border whitespace-nowrap ${isLocalMode ? 'border-sky-800 text-sky-400' : 'border-emerald-800 text-emerald-400'}`}>
                        {isLocalMode ? t.mode_local : t.mode_hybrid}
                        </span>
                    </div>
                    <h1 className={`text-3xl md:text-5xl font-black tracking-tight ${titleColor} mb-2 break-words leading-tight`}>
        ...
        {/* JSON Toggle */}
        <div className="mt-8 border-t border-zinc-800 pt-6" data-html2canvas-ignore="true">
            <button 
